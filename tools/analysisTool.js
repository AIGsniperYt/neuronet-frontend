  ...
  if (sourceEditor && editorToolbar) {
    ...
    allBtns.forEach(btn => {
      ...
      btn.addEventListener("click", (e) => {
        ...
        // Log selection state BEFORE formatting
        const selBefore = window.getSelection();
        console.log('[FORMAT] Before:', command, 'rangeCount:', selBefore?.rangeCount, 'collapsed:', selBefore?.isCollapsed);
        if (selBefore?.rangeCount > 0) {
          const r = selBefore.getRangeAt(0);
          console.log('  start:', r.startContainer.textContent?.substring(0, 30), 'offset:', r.startOffset);
          console.log('  end:', r.endContainer.textContent?.substring(0, 30), 'offset:', r.endOffset);
        }
        
        execFormat(command, value);
        
        // Log selection state AFTER formatting
        setTimeout(() => {
          const selAfter = window.getSelection();
          console.log('[FORMAT] After:', command, 'rangeCount:', selAfter?.rangeCount);
          sourceEditor.focus();
        }, 0);
      });
    });
